...
void lbrCreateMap(LbrMap *p_map, usize capacity, usize key_size, usize value_size,
	...
	p_map->data = lbrAllocCallbackAllocate(&alloc_callback, capacity * (key_size + value_size));
...
void lbrMapRemove(LbrMap* p_map, void* p_key) {
	...
	if (p_map->equals(p_key, p_map->data + (idx * p_map->key_size))) {
		...
		memcpy(p_map->data + (p_map->capacity * p_map->key_size) + (idx * p_map->value_size), 0, p_map->value_size);
		p_map->length--;
	}
}
